...
public:
   const static uint32_t N = static_cast<uint32_t>(TAirport::Last_);
    ...
    {
        return arr.at(static_cast<uint32_t>(airport));
    }

    // добавить данный элемент
    void Insert(TAirport airport)
    {
        ++arr[static_cast<uint32_t>(airport)];
    }

    // удалить одно вхождение данного элемента
    void EraseOne(TAirport airport)
    {
        if(arr[static_cast<uint32_t>(airport)] != 0)
            --arr[static_cast<uint32_t>(airport)];
    }
    ...
    {
        arr[static_cast<uint32_t>(airport)] = 0;
    }
    ...
    Items GetItems() const
    {
        ...
        {
            result[i] = make_pair(static_cast<TAirport>(i), arr[i]);
        }
        return result;
    }

private:
    array<size_t, N> arr = {0};
};
